...
coloredlogs.install(fmt="[%(levelname)8s] %(asctime)s %(message)s", datefmt='%H:%M:%S', field_styles={'asctime': {'color': 'magenta', 'bright': True}, 'hostname': {'color': 'magenta'}, 'levelname': {'color': 'black', 'bold': True}, 'name': {'color': 'blue'}, 'programname': {'color': 'cyan'}}, level='DEBUG', logger=rootLogger)
...
class Jeeves(discord.Client):
    def __init__(self):
        ...
        self.prefix = prefix
        self.avatars = ['http://i.imgur.com/5cREXtw.png', 'http://i.imgur.com/Kz3Ndng.png', 'http://i.imgur.com/86BR273.png', 'http://i.imgur.com/hhRNuKJ.png']
        self.email = 'PUT YOUR EMAIL HERE'
        self.password = 'PUT YOUR PASSWORD HERE'
        self.bot_token = 'PUT A BOT TOKEN HERE'
        ...
        self.bot_http = HTTPClient(None, proxy=None, proxy_auth=None, loop=asyncio.get_event_loop())
        ...
        # self.twitAPI = TwitterAPI('TWITTER CLIENT STUFF',
                                  # 'GOES HERE',
                                  # 'I DONT REMEMBER',
                                  # 'THE SPEDCIFIC NAMES')
    ...
    async def do_search(self, guild_id, **kwargs):
        ...
        return await self.http.request(Route('GET', f'/guilds/{guild_id}/messages/search{string_query}'))
    ...
    async def cmd_forceban(self, guild, this_id):
        await self.http.ban(this_id, guild.id, delete_message_days=0, reason='force ban')
        return Response(':thumbsup:', reply=True)
    ...
    async def cmd_outputlogs(self, channel, message, count):
        ...
        async for msg in channel.history(limit=count, before=message):
            output += '{}#{} ({}) - {}\n{}\n\n'.format(msg.author.name, msg.author.discriminator, msg.author.id, msg.created_at.strftime("%H:%M:%S on %a %b %d"), msg.clean_content)
        write_file(datetime.utcnow().strftime("%H_%M_%S_%a_%b_%d"), [output.encode('ascii', 'ignore').decode('ascii')])
        return Response(':thumbsup:', reply=True)

    async def cmd_steal(self, string_avi, emoji_name="stolen-emote", server_id=298778712709529603):
    ...
    async def cmd_cls(self, message, channel):
        async for msg in channel.history(500, before=message).filter(lambda m: m.author == self.user):
    ...
    async def cmd_changetracker(self, message, mentions):
        ...
        write_file('nowplaying.txt', ['{0}{1}{0}'.format(' ' * (len(this) + 200), this)])
        ...
        if member:
            ...
            try:
                ...
                if not reason:
                    ...
                    em.add_field(name='User Banned from Server:', value=f'-Reason Unknown-', inline=False)
                ...
                if tag_name.lower() == tag.lower():
                    if self.tags[tag][0]:
                        ...
                        if "eval" in tag_flags:
                            ...
                            if resp:
                                await self.safe_edit_message(channel, content=clean_bad_pings(resp), send_if_fail=True)
                            return
                    return Response(clean_bad_pings(self.tags[tag][1]))
            raise CommandError('Tag doesn\'t exist')
            
    async def on_raw_reaction_add(self, payload):
        if payload.user_id == 294882584201003009:
            rootLogger.info(f"found giveaway on {self.get_guild(payload.guild_id).name}. Will react if >10 votes in 360s...\n")
            for x in range(360):
                await asyncio.sleep(1)
                
                msg = discord.utils.get(self._connection._messages, id=payload.message_id)
                if msg and msg.reactions and msg.reactions[0].count > 10:
                    await self.http.add_reaction(payload.message_id, payload.channel_id, payload.emoji._as_reaction())
                    rootLogger.info(f"Reaction added to giveaway on {self.get_guild(payload.guild_id).name} after {x} seconds.\n")
                    return
            rootLogger.debug(f"giveaway on {self.get_guild(payload.guild_id).name} didnt reach minimum votes for reaction." + "\n")
                
    async def on_member_update(self, before, after):
        if before.id == self.fm_bot_watcher and before.game != after.game and after.game:
            this = after.game.name.encode('ascii', 'ignore').decode('ascii')
            self.now_playing = this
            write_file('nowplaying.txt', '{0}{1}{0}'.format(' ' * (len(this) + 600), this))
    ...
    async def on_message(self, message, edit=False):
        if isinstance(message.channel, discord.DMChannel):
            if message.author != self.user:
                if not (self.user.get_relationship(message.author.id) and self.user.get_relationship(message.author.id).type.friend):
                    
                    important_guilds = {'rhinos place 3': 77514836912644096,
                                        ...
                                        'Discord Hotline': 204100839806205953
                                        }
                    
                    if not any(guild in list(important_guilds.values()) for guild in [guild.id for guild in self.guilds if message.author.id in [member.id for member in guild.members]]):
                        await self.http.ack_message(message.channel.id, message.id)
                        rootLogger.info(f"DM from {message.author.name}, not friend so acking")
                    else:
                        rootLogger.info(f"DM from {message.author.name}, not friend but in acceptable server so not acking")
                else:
                    rootLogger.info(f"DM from {message.author.name}")
                dm_tracking_channel = discord.utils.get(self.get_all_channels(), id=398966481406263297)
                self.divider_content = '__                                                                                                          __\n\n'
                jump_embed = discord.Embed(colour=discord.Colour(0x36393F), description=f"[click to jump]({message.jump_url})")
            ...
            if message.author != self.user:
                ...
                if message.attachments:
                    ...
                    if edit: 
                        await self.safe_send_message(dm_tracking_channel, embed=jump_embed, content=f'{self.divider_content}**EDITED MSG From:** *{message.author.mention}* in Group DM {message.channel.name}:\n```{message.clean_content}```\n~Attachments: {", ".join([attachment.url for attachment in message.attachments])}\nID: `{message.author.id}`')
                    else:
                        await self.safe_send_message(dm_tracking_channel, embed=jump_embed, content=f'{self.divider_content}**From:** *{message.author.mention}* in Group DM {message.channel.name}:\n```{message.clean_content}```\n~Attachments: {", ".join([attachment.url for attachment in message.attachments])}\nID: `{message.author.id}`')
            ...
            if edit: return
            if message.author != self.user: 
                if not message.author.id in [135288293548883969, 77511942717046784] and not message.author.bot:
                    ...
                    msg = None
                    if message.guild.me in message.mentions or (message.mention_everyone and message.guild.id != 298778712709529603) or (set(message.guild.me.roles).intersection(message.role_mentions) and message.guild.id != 298778712709529603):
                        jump_embed = discord.Embed(colour=discord.Colour(0x36393F), description=f"[click to jump]({message.jump_url})")
                        if not message.mention_everyone: await self.safe_send_message(discord.utils.get(self.get_all_channels(), id=304046409185755146), embed=jump_embed, content='\nmentioned on %s(%s) in <#%s> by %s (%s)```%s```' % (message.guild.name, message.guild.id, message.channel.id, message.author.mention, message.author.id, message.clean_content))
                        ...
                        if message.guild.id not in ['298778712709529603']:
                            ...
                            if re.search(regex, message.clean_content, re.IGNORECASE):
                                jump_embed = discord.Embed(colour=discord.Colour(0x36393F), description=f"[click to jump]({message.jump_url})")
                                await self.safe_send_message(discord.utils.get(self.get_all_channels(), id=304046409185755146), embed=jump_embed, content='\nmentioned on %s(%s) in <#%s> by %s (%s)```%s```' % (message.guild.name, message.guild.id, message.channel.id, message.author.mention, message.author.id, message.clean_content))
                ...
                if not handler:
                    ...
                    if tag_name in self.tags:
                        if self.tags[tag_name][0]:
                            ...
                            if "eval" in tag_flags:
                                ...
                                if resp:
                                    await self.safe_edit_message(message.channel, content=clean_bad_pings(resp), send_if_fail=True)
                                return
                                
                        await self.safe_edit_message(message.channel, content=clean_bad_pings(self.tags[tag_name][1]), send_if_fail=True)
                    ...
                    try:
                        ...
                        if response and isinstance(response, Response):
                            ...
                            if len(content) > 2000:
                                split = [content[i:i + 2000] for i in range(0, len(content), 2000)]
                                for x in split:
                                    await self.safe_send_message(message.channel, content=x)
                                await self.safe_delete_message(message)
                            elif response.send_message:
                                if response.reply:
                                    content = '**(╭ರ_•)**-%s' % content
                                await self.safe_send_message(message.channel, content=content, expire_in=response.delete_after)
                            else:
                                if response.reply:
                                    content = '**(╭ರ_•)**-%s' % content
                                await self.safe_edit_message(message, new_content=content, expire_in=response.delete_after, send_if_fail=True)
